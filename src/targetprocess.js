...
class Targetprocess {
    ...
    _getOptions(resource, select, where, skip, take) {
        return {
            url: `${this._url}/api/v2/${resource}?select=${select || ''}
            &where=${where || 'true'}&skip=${skip}&take=${take}&token=${this._token}`
        };
    }
    ...
    *validate() {
        try {
            var loggedUserResponse = yield request({
                url: `${this._url}/api/v1/Users/loggedUser?token=${this._token}&format=json&
                include=[IsActive,DeleteDate,IsAdministrator]`
            });
        } catch (e) {
            return {error: [e]};
        }
    ...
    *createWebHook(toolToken, url) {
        ...
        var profile = {
            ...
            Settings: {
                ...
                UseTemplate: false,
                Template: '',
                Filter: this._config.projects ? _.map(this._config.projects,
                    project=>_.isString(project) ? `ProjectName == "${project}"` : `ProjectID == ${project}`
                ).join(' or ') : '',
                OnCreate: true,
                OnUpdate: true,
                OnDelete: true,
